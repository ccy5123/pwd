#!/usr/bin/env python3
"""
Storage lipid-water 분배계수 계산 (openCOSMO-RS_py) v4-storage
muscle/albumin 버전에서 변경점:
  (1) 용매: 단백질 capped residue 혼합 -> NTG(trinonanoylglycerol) 단일 분자
      → 조성(mole fraction) 불필요. 순수 용매(x=1)에 무한희석.
      근거: Geisler 2015 (ES&T 49,5538) — COSMOtherm 계산에 triolein/NTG 사용,
            둘이 거의 동일(평균차 0.25 log), NTG가 작아 계산 빠름 → NTG 채택.
            Geisler 2012 — 지방산 조성 무관, 모든 storage lipid 단일상 취급 가능.
  (2) 검증셋: STORAGE247 (Geisler 2012 Table SI-1, 전부 37C)
나머지(openCOSMO-RS API, MD5 캐시키, TEMP_K=310.15, ORCA 설정)는 muscle 원본 동일.

성능 기대(Geisler 2015 COSMOtherm 기준): 단순 rmse~0.45, H결합donor~0.35,
  복잡~0.71. storage lipid는 비구조적(유기용매類) 상이라 4상 중 COSMO가 가장 잘 맞음.
한계: openCOSMO-RS 24a neutral 전용. 이온화 화합물은 중성종 기준(논문도 동일).
"""
import os, math
import numpy as np
...
TEMP_K = 310.15  # 37C — Geisler 2012 측정온도 (muscle/albumin과 동일)

# storage lipid 용매 모델: NTG = trinonanoylglycerol (C9 fatty acid x3 + glycerol)
NTG_SMILES = "CCCCCCCCC(=O)OCC(COC(=O)CCCCCCCC)OC(=O)CCCCCCCC"
# (대안: triolein "CCCCCCCC/C=C\\CCCCCCCC(=O)OCC(COC(=O)CCCCCCCC/C=C\\CCCCCCCC)OC(=O)CCCCCCCC/C=C\\CCCCCCCC")
...
def calculate_partition_coefficient(solute_orcacosmo, ntg_orcacosmo, water_orcacosmo, T=TEMP_K, c_unit=0.0):
    """log K_storage/water 계산. 용매는 순수 NTG (x=1), 물도 순수 (x=1)."""
    try:
        crs = COSMORS(par=openCOSMORS24a())
        lng_w = lng_inf(crs, solute_orcacosmo, [water_orcacosmo], [1.0], T)   # 물에서
        lng_l = lng_inf(crs, solute_orcacosmo, [ntg_orcacosmo],   [1.0], T)   # NTG(지질)에서
        ln_Kx = lng_w - lng_l
...
def find_orcacosmo_file(cache_key_str):
    """캐시 키로 .orcacosmo 찾기 (muscle 원본과 동일)"""
...
def validate_storage247(label="openrs_storage_v4"):
    print("=== openCOSMO-RS Storage lipid 검증 v4 (37C, 용매=NTG) ===")
    water = str(CACHE / "WATER.orcacosmo")
    ntg   = str(CACHE / "NTG.orcacosmo")
    for f,nm in [(water,"WATER"),(ntg,"NTG")]:
        if not Path(f).exists():
            print(f"[ERROR] {nm}.orcacosmo 없음 → calc_sigma_storage_v4.py 먼저 실행"); return None
    print(f"Solvent: NTG={ntg}\n         WATER={water}")

    import hashlib
    exp_vals, pred_vals, names, rows = [], [], [], []
    for name, exp_k, smiles in STORAGE247:
        key = hashlib.md5(smiles.encode()).hexdigest()[:16]
        oc = find_orcacosmo_file(key)
        if oc is None:
            print(f"[skip] {name}: .orcacosmo 없음"); continue
        log_k = calculate_partition_coefficient(oc, ntg, water, TEMP_K, c_unit=0.0)
...
if __name__ == "__main__":
    ...
    if len(sys.argv)>1 and sys.argv[1]=="validate":
        ...
        print("Usage: python cosmo_kpw_openrs_storage_v4.py validate")
